import { BasicInfoView } from "@/features/memberManageNew/components/organisms/tabs/BasicInfoView"; // Import the new View component
...
const MemberDetailPage = () => {
  ...
  const { id } = useParams(); // Get member ID

  // Navigate to the update page
  const goToUpdate = () => {
    navigate(`${route.memberUpdate}/${id}`); // Assuming this is your update route
  };
  ...
  return (
    <div className="w-full max-w-[1044px]">
      ...
      <div className="mt-6 rounded-lg bg-white">
        <BasicInfoView userType="student" data={mockInitialData} />
      </div>
    </div>
  );
};
